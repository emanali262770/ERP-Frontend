        ...
        try {
            if (editingCategory) {
                setCategories(categories.map(c =>
                    c._id === editingCategory._id ? { ...payload, _id: editingCategory._id, createdAt: editingCategory.createdAt } : c
                ));
                toast.success("✅ Promotion updated!");
            } else {
                setCategories([
                    ...categories,
                    { ...payload, _id: Date.now().toString(), createdAt: new Date().toISOString() }
                ]);
            ...
            console.error(error);
            toast.error(`❌ Failed to ${editingCategory ? "update" : "add"} promotion.`);
        } finally {
            setLoading(false);
        }
    };
      ...
      <CommanHeader/>
            <div className="px-6 mx-auto">
                <div className="flex justify-between items-center mb-4">
                    <div>
                        <h1 className="text-2xl font-bold text-newPrimary">Promotion Details</h1>
                    </div>
                    <button
                ...
                <div className="rounded-xl shadow border border-gray-100 overflow-hidden">
                    <div className="overflow-x-auto">
                        <div className="min-w-full">
                            {/* Header Row */}
                            <div className="hidden lg:grid grid-cols-[80px_1fr_1fr_150px_150px_150px] gap-6 bg-gray-50 py-3 px-6 text-xs font-medium text-gray-500 uppercase">
                            ...
                            <div className="flex flex-col divide-y divide-gray-100">
                                {categories.length === 0 ? (
                                    <div className="text-center py-4 text-gray-500">No promotions found.</div>
                                ) : (
                                    categories.map((category, index) => (
                                        <div
                                            key={category._id}
                                            className="grid grid-cols-1 lg:grid-cols-[80px_1fr_1fr_150px_150px_150px] gap-4 lg:gap-6 bg-white p-4 rounded-lg shadow-sm hover:shadow-md transition border border-gray-100"
                                        >
                                            ...
                                            {/* Details */}
                                            <div className="text-sm text-gray-700">{category.details}</div>

                                            {/* Start Date */}
                                            <div className="text-sm text-gray-500">{category.startDate}</div>

                                            {/* End Date */}
                                            <div className="text-sm text-gray-500">{category.endDate}</div>
                    ...
                    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-end z-50">
                        ...
                        >
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                {/* Details */}
                                <div>
                                    <label className="block text-gray-700 font-medium mb-4">Details</label>
                                    <textarea
                                        value={details}
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className={`text-sm font-medium ${isEnable ? "text-green-600" : "text-gray-500"}`}>
